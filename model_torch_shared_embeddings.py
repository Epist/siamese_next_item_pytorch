...
class SiameseRecNet(torch.nn.Module):
	...
	def forward(self, input_list):
		...
		for i, cur_prev_item_input in enumerate(prev_item_embeddings): #Previous items
			hidden = self.prev_items_hidden_layers[i]
			cur_prev_item_input_embedding = self.item_embedding(cur_prev_item_input)
			left_embeddings += hidden(self.post_embedding_nonlinearity(DO(cur_prev_item_input_embedding)))
			right_embeddings += hidden(self.post_embedding_nonlinearity(DO(cur_prev_item_input_embedding)))
