...
class SiameseRecNet(torch.nn.Module):
	def __init__(self, num_users, num_items, num_previous_items, num_hidden_layers, num_hidden_units, embedding_size, activation_type, dropout_prob, use_masking, use_gpu):
		...
		#self.dist = nn.modules.distance.PairwiseDistance(p=1)
	...
	def forward(self, input_list):
		...
		#left_embeddings = [self.next_item_input_layer(self.post_embedding_nonlinearity(self.item_embedding(input_list[1])))] #Left candidate item
		#right_embeddings = [self.next_item_input_layer(self.post_embedding_nonlinearity(self.item_embedding(input_list[2])))] #Right candidate item
		left_embeddings = [self.item_embedding(input_list[1])] #Left candidate item
		right_embeddings = [self.item_embedding(input_list[2])] #Right candidate item

		input_size = list(input_list[0].size())[0]
		#print(input_list[0])
		#user_input_embedding = self.user_input_layer(self.post_embedding_nonlinearity(self.user_embedding(input_list[0]))) #Variable(torch.zeros([input_size, 64]), requires_grad=False).cuda() #self.user_embedding(input_list[0])
		user_input_embedding = self.user_embedding(input_list[0])
		#print(user_input_embedding)
		left_embeddings.append(user_input_embedding)
		right_embeddings.append(user_input_embedding)
		...
		for i, cur_prev_item_input in enumerate(prev_item_inputs): #Previous items
			...
			if self.use_masking:
				post_embedding = cur_prev_item_input_embedding.t()
				cur_mask = prev_item_masks[i]
				masked_embedding = post_embedding.t() * cur_mask.expand_as(post_embedding).t()
				left_embeddings.append(masked_embedding)
				right_embeddings.append(masked_embedding)
			else:
				#post_embedding = self.prev_items_hidden_layers[i](self.post_embedding_nonlinearity(cur_prev_item_input_embedding))
				#post_embedding = self.next_item_input_layer(self.post_embedding_nonlinearity(cur_prev_item_input_embedding))
				post_embedding = cur_prev_item_input_embedding
				left_embeddings.append(post_embedding)
				right_embeddings.append(post_embedding)
		...
		left = self.DO(torch.cat(left_embeddings, dim=1))
		right = self.DO(torch.cat(right_embeddings, dim=1))
		#print("input size", left.size())

		#print("INPUT DO TRAINING: ", self.DO.training)
		#print("MODEL DO TRAINING: ", self.siamese_layers_dict["dropout_"+str(0)].training)

		left_output = self.siamese_half(left)
		right_output = self.siamese_half(right)
